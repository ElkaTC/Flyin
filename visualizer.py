...
from models import Color, Drone
from simulator import Simulator


class Menu:
    """Handles the map selection menu,
    including file discovery and user input."""
    def __init__(self, screen: pygame.Surface) -> None:
        ...
        self.selected = 0
        self.chosen_map: Optional[str] = None

    def map_name(self, map_path: str) -> str:
        """Formats the raw map file path into a clean,
    ...
    def handle_event(self, event: pygame.event.Event) -> None:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key in (pygame.K_UP, pygame.K_z):
                ...
                self.chosen_map = os.path.join(
                    "maps", self.maps[self.selected]
                )


class Renderer:
    """Manages the Pygame window context, graphics rendering,
    and the main game loop."""
    def __init__(self, default_map: Optional[str] = None) -> None:
        pygame.init()
        self.width = 1920
        self.height = 1080
        ...
        if default_map:
            ...
            if success:
                self.state = 'GAME'
                self.menu.chosen_map = default_map
            else:
                print(f"ERROR: Map can't be loaded '{default_map}'")
                pygame.quit()
                sys.exit(1)

        self.wallpaper = pygame.image.load(
            'graphics/wallpaper.png'
        ).convert()
        self.wallpaper = pygame.transform.scale(
            self.wallpaper, (self.width, self.height)
        )
        self.wallpaper_menu = pygame.image.load(
            'graphics/wallpaper_menu.png'
        ).convert()
        self.wallpaper_menu = pygame.transform.scale(
            self.wallpaper_menu, (self.width, self.height)
        )
        self.drone_sprite = pygame.image.load(
            'graphics/drone.png'
        ).convert_alpha()
        self.drone_sprite = pygame.transform.scale(
            self.drone_sprite, (111, 111)
        )
        self.stats_font = pygame.font.Font(
            'graphics/Kids_Word.otf', 20
        )

        self.running = True
        self.drone_visual_positions: Dict[Drone, List[float]] = {}
    ...
    def run(self) -> None:
        ...
        while self.running:
                ...
                if self.state == 'GAME':
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_SPACE:
                            ...
                            and not self.simulator.game_started
                        ):
                            self.simulator.step()
                            last_step_time = now
                            ...
                            if self.menu.chosen_map is not None:
            ...
            if self.state == 'MENU':
                ...
                self.menu.draw()
            elif self.state == 'GAME' and self.simulator.graph is not None:
                self.screen.blit(self.wallpaper, (0, 0))

                text_turn = self.menu.font.render(
                    f"Turn : {self.simulator.graph.turn}",
                    True, (255, 255, 255)
                )
                self.screen.blit(text_turn, (1550, 30))

                state_text = (
                    'Auto' if self.simulator.game_started else 'Manual'
                )
                text_mode = self.menu.font.render(
                    f"Mode : {state_text}",
                    True, (255, 255, 255)
                )
    ...
    def draw_areas(self) -> None:
        ...
        for area in graph.areas.values():
            ...
            if area.color == 'none' or area.color is None:
                draw_color = Color.GREEN if area.role == 'start_hub' else (
                    Color.RED if area.role == 'end_hub' else Color.WHITE
                )
            else:
            ...
            cap_text = (
                ...
                if area.role == 'hub'
                else f'{nb_drones}/{len(graph.drones)}'
            )
    ...
    def draw_drones(self) -> None:
        """Updates and draws drones using localized
        ...
        for drone in graph.drones:
            if drone.current_connection:
                if drone.target_area is None:
                    continue

                start_area = (
                    drone.current_connection.area2
                    if drone.current_connection.area1 == drone.target_area
                    else drone.current_connection.area1
                )
                total_cost = drone.current_connection.cost_to(
                    drone.target_area
                )
                progress = (
                    drone.travel_progress / total_cost
                    if total_cost > 0 else 1.0
                )
                raw_x = (
                    start_area.pos[0]
                    + (drone.target_area.pos[0] - start_area.pos[0])
                    * progress
                )
                raw_y = (
                    start_area.pos[1]
                    + (drone.target_area.pos[1] - start_area.pos[1])
                    * progress
                )
            else:
                current_area = drone.current_area
            ...
            rect = self.drone_sprite.get_rect(
                center=(int(new_vis_x), int(new_vis_y))
            )
            self.screen.blit(self.drone_sprite, rect)

    def get_offset(self) -> tuple[int, int]:
        """Calculates centering offsets based on
        extreme map coordinates to bound graphics onto the screen canvas."""
        graph = self.simulator.graph
        if graph is None:
            return 0, 0
        positions_x = [
            area.pos[0] for area in graph.areas.values()
        ]
        positions_y = [
            area.pos[1] for area in graph.areas.values()
        ]
